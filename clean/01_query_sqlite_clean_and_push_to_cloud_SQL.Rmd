...
```{r}
# format battery update
library(stargazer)
battery_html_table <- 
  stargazer(battery_life_df, type = "html", 
            title = "Battery Life of LS & LL Units", 
            summary = FALSE) %>% 
  paste(., collapse = "")
...
Source the Rmd file that writes the daily report, and wait a minute.
```{r}
source("02_daily_report.Rmd")
Sys.sleep(time = 60)
```
...
mime() %>% 
  ...
  text_body("See the attached PDF.") %>% 

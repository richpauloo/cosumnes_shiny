...
# v = lines[[1]] # non-MW5
# v = lines[[6]]  # MW5
# v = lines[[195]] # non-equal ... need to save extra baro without filtering it out
...
dfs <- lapply(lines[1:180], get_data) # temp until non-equal issue is fixed
...
mw_data   <- rename(mw_data,   level_mw = level,   mw_id = id)
...
elev$mw_id <- c("1065731","1055097","1065751","2083804")

# find baro-adjusted water level for matching date time in manually measured `date_of_last_dtw_measurement`
ll_1065731 <- filter(adj_data, mw_id == "1065731" & dt <= mdy_hm("10/12/2018 14:54"))[1, "adj_level"]
ll_1055097 <- filter(adj_data, mw_id == "1055097" & dt <= mdy_hm("10/12/2018 11:31"))[1, "adj_level"]
ll_1065751 <- filter(adj_data, mw_id == "1065751" & dt <= mdy_hm("10/12/2018 13:37"))[1, "adj_level"]
ll_2083804 <- filter(adj_data, mw_id == "2083804" & dt <= mdy_hm("10/12/2018 16:03"))[1, "adj_level"]
...
# create a key for which wells subtract 9.5 meters for barometic pressure
adj_data <- left_join(adj_data, elev, by = "mw_id") 
